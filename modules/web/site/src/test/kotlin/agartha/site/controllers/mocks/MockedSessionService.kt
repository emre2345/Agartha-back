...
class MockedSessionService : ISessionService {
    ...
    override fun startSession(userId: String, discipline: String, practition: String, intention: String): Int {
        val first = practitionerList.filter {
            it._id.equals(userId)
        }.first()

        val nextIndex = first.sessions.count() + 1
        first.sessions.plus(SessionDBO(nextIndex, discipline, practition, intention))
        return nextIndex
